...
class Episode:
    ...
    info: Dict[str, Any]
    full_res_obs: Optional[torch.FloatTensor] = None
    ...
    def load(cls, path: Path, map_location: Optional[torch.device] = None) -> Episode:
        return cls(
            **{
                k: v.div(255).mul(2).sub(1) if "obs" in k else v
    ...
    def save(self, path: Path) -> None:
        ...
        d = {k: v.add(1).div(2).mul(255).byte() if "obs" in k else v for k, v in self.__dict__.items()}
