...
def main():
    ...
    for task in tqdm(list(SINGLE_STAGE_TASK_DATASETS.keys())):
        ds_path = get_ds_path(task, ds_type="human_im", return_info=False)
        f = h5py.File(ds_path, "r")
        demo_keys = list(f['data'].keys())
        num_demos = len(demo_keys)
        split_idx = int(0.9 * num_demos)
        train_keys = demo_keys[:split_idx]
        test_keys = demo_keys[split_idx:]
